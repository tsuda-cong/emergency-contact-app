...
どちらのリンクでも、氏名と生年月日が既存の回答（削除済みを除く）と一致する場合は登録されず、「すでに登録されています。内容を変更したい場合は長老にご連絡ください。」と表示される。氏名は全角・半角とスペースの有無の違いを吸収して比較する（異体字は区別する）。公開フォームからの上書きはできないため、内容の変更は更新リンクで行う。
